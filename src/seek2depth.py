...
class rgb_human_detection_node():
    def __init__(self):
        ...
        self.bboxSub = rospy.Subscriber("/detections/thermal", Detector2DArray, self.get_bbox_IR, queue_size=1)
        ...
        self.IR_intrinsics = rs2.intrinsics()
        self.IR_intrinsics.width = 320
        self.IR_intrinsics.height = 240
        self.IR_intrinsics.ppx = 1.6001014057027311e+02
        self.IR_intrinsics.ppy = 1.1921412901927177e+02
        self.IR_intrinsics.fx = 3.3092071512439185e+02
        self.IR_intrinsics.fy = 3.3213288273601660e+02
        self.IR_intrinsics.model = rs2.distortion.brown_conrady
        self.IR_intrinsics.coeffs = [-2.3860417160339289e-01, -1.0059872606293430e+00,
                                     -1.2342103021496931e-02, -6.4050866560307817e-03,
                                      2.6056479258743481e+00]

        self.IR_to_depth_extrin = rs2.extrinsics()
        self.IR_to_depth_extrin.translation = [-0.0147247, 0.000350991, 0.000350991]
        self.IR_to_depth_extrin.rotation = [1, 0, 0, 0, 1, 0, 0, 0, 1]

        self.depth_to_IR_extrin = rs2.extrinsics()
        self.depth_to_IR_extrin.translation = [0.0147247, -0.000350991, -0.000350991]
        self.depth_to_IR_extrin.rotation = [1, 0, 0, 0, 1, 0, 0, 0, 1]
    ...
    def project_IR_to_depth(self, depth_image, from_pixel):
        to_pixel = [0.0,0.0]
        min_point = rs2.rs2_deproject_pixel_to_point(self.IR_intrinsics, from_pixel, self.depth_min)
        min_transformed_point = rs2.rs2_transform_point_to_point(self.depth_to_IR_extrin, min_point)
        ...
        max_point = rs2.rs2_deproject_pixel_to_point(self.IR_intrinsics, from_pixel, self.depth_max)
        max_transformed_point = rs2.rs2_transform_point_to_point(self.depth_to_IR_extrin, max_point)
        ...
        while rs2.is_pixel_in_line(p, start_pixel, end_pixel):
            ...
            transformed_point = rs2.rs2_transform_point_to_point(self.IR_to_depth_extrin, point)
            projected_pixel = rs2.rs2_project_point_to_pixel(self.IR_intrinsics, transformed_point)
        ...
        # bridge_seek = CvBridge()
        # seek_image = bridge_seek.imgmsg_to_cv2(seek_image, desired_encoding="passthrough")
        # if self.scat_rgb != None:
        #     self.scat_rgb.remove()
        # plt.imshow(seek_image)
        # self.scat_rgb = plt.scatter(from_pixel[0],from_pixel[1])
        # plt.show()

        # if self.scat_rgb != None:
        #     self.scat_rgb.remove()
        # plt.imshow(self.depth_image)
        # self.scat_rgb = plt.scatter(int(to_pixel[0]),int(to_pixel[1]))
        # plt.show()

        return to_pixel

    def plot_mult(self,image, pix):
        if self.scat_rgb != None:
            self.scat_rgb.remove()
        plt.imshow(image)
        self.scat_rgb = plt.scatter(int(pix[0]),int(pix[1]), c="white")
        plt.show()
    ...
    def get_bbox_IR(self, detector_2d_array):
        ...
        if self.depth_image is None:
            ...
            return
        elif self.IR_intrinsics is None:
            return
        else:
        ...
        for detection in temp_obs:
            ...
            depth_points = []
            for IR_point in IR_points:
                depth_flattened = temp_image.flatten()
                depth_point = self.project_IR_to_depth(depth_flattened, IR_point)
